...
export class TslaComponent {
  ...
  generateGraph() {
    ...
    const url = `https://finnhub.io/api/v1/stock/candle?symbol=TSLA&resolution=D&from=${oneYearAgoTimestamp}&to=${currentTimestamp}&token=cc676uaad3i9rj8tb1s0`;
  
    this.http.get<any>(url).subscribe(data => {
      const chartData = data.c;
      console.log(data);
      const chartLabels = data.t.map((timestamp: number) => this.formatTimestamp(timestamp));
      this.lineChartData = [{ data: chartData, label: `MICROSOFT Stock Price` }];
  ...
  ngOnInit() {
    this.userService.tsla().subscribe((data)=>{
